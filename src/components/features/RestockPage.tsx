...
export const RestockPage: React.FC = () => {
  const { products, suppliers, recordTransaction, settings } = useInventory();
  ...
  const cartTotal = cart.reduce((sum, item) => sum + (item.product.costPrice * item.quantity), 0);
        ...
        <div style={{ marginTop: '2rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            <h3 style={{ fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <PackagePlus size={20} /> Restock List ({cart.length})
            </h3>
            <button onClick={() => setCart([])} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', fontSize: '0.85rem' }}>Clear All</button>
          </div>
        ...
        {cart.length > 0 && (
          ...
          >
            <div style={{ maxWidth: '800px', margin: '0 auto' }}>
              ...
              >
                {isProcessing ? 'Processing...' : `Confirm Restock (${cart.length} Items)`}
              </button>
            </div>
          </motion.div>
        )}
